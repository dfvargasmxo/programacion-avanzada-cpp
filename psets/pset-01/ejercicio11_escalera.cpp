...
struct Escalera {
    ...
    bool esSegura() {
        if (numeroPeldanos <= 12) { return true;} else {
            return false;
        }
    }
};
...
int main() {
    ...
    Escalera b;
    b.numeroPeldanos = 15;
    std::cout << "Escalera de " << a.numeroPeldanos << " peldanos: ";
    if (a.esSegura()) {
        std::cout << " es segura" << std::endl;} else {
        std::cout << " no es segura" << std::endl;}
    std::cout << "Escalera de " << b.numeroPeldanos << " peldanos: ";
    if (b.esSegura()) {
        std::cout << " es segura" << std::endl;} else {
        std::cout << " no es segura" << std::endl;}
