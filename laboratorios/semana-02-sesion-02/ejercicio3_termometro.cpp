...
public:
    Termometro(double celsiusInicial) {
        ...
        bool set_celsius(double celsiusNuevo) {
            if (celsius < -273.25) {return false;} else {
            gradosCelsius = 0.0;
            return true;

        } }
        
        if (!set_celsius(celsiusInicial)) {
            gradosCelsius = 0.0;
            std::cout << "Aviso: temperatura invalida, se usó 0 por defecto" << std::endl;
        } else {
